...
1. Each request consumes one token. When a request arrives, we check if there are enough tokens in the bucket. Figure 5 explains how it works.
...
- We can have multiple Rate Limiter servers(distributed). In case client requests goes to different Rate Limiter servers, we can have Redis to maintain the Counter and queue. 
- Redis inherently does not suport atomicity but we have solutions to ensure Atomicity in redis
  ...
  - Other popular options: Lua script and sorted sets data structure in Redis